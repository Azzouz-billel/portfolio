...
/* Aura Cursor — Originkit fluid-simulation cursor trail.
 * Ported from the TSX original to JSX; the label overlay and native-cursor
 * hiding were dropped (this site already has its own cursor torch), and the
 * frame is expected to be a fixed fullscreen, pointer-events-none layer. */
...
export function AuraCursor(props) {
  const {
  ...
  // The component's own frame, kept in flow purely so the hover test has a
  // real rect to test against.
  ...
  // framebuffers on every prop change would be brutal, so the loop reads
  ...
  useEffect(() => {
    ...
    const compileShader = (type, source, keywords) => {
      const withDefines = keywords ? keywords.map((k) => `#define ${k}\n`).join('') + source : source
    ...
    const shouldSplat = () => isInsideHoverZone(pointer.clientX, pointer.clientY)
    ...
    const onPointerMove = (e) => {
      ...
      pointer.moved =
        !firstMove && (Math.abs(pointer.deltaX) > 0 || Math.abs(pointer.deltaY) > 0)
      if (firstMove) {
  ...
  // The frame is what the fluid fills and what the hover zone measures.
  // pointer-events: none — the solver listens on the window instead, so it
  // never swallows a click meant for whatever sits under it.
  return (
    <div
      ref={anchorRef}
      style={{
